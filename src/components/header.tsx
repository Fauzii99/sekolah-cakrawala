...
export function Header({ home = false }: { home?: boolean }) {
  ...
  return <header className={`site-header ${solid ? "is-solid" : "is-clear"}`}><nav className="nav-shell" aria-label="Navigasi utama">
    ...
    <div className={`nav-links ${open ? "open" : ""}`}><Link onClick={()=>setOpen(false)} href="/#cerita">Tentang</Link><Link onClick={()=>setOpen(false)} href="/#program">Jurusan</Link><Link onClick={()=>setOpen(false)} href="/benefit" aria-current={pathname === "/benefit" ? "page" : undefined} style={pathname === "/benefit" ? { textDecoration: "underline", textUnderlineOffset: "6px" } : undefined}>Benefit</Link><Link onClick={()=>setOpen(false)} href="/#berita">Jurnal</Link><Link onClick={()=>setOpen(false)} href="/#kontak">Kontak</Link><Link onClick={()=>setOpen(false)} className="nav-ppdb" href="/ppdb">PPDB 2026 <span>↗</span></Link></div>
  </nav></header>;
}
