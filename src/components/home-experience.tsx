...
export function HomeExperience() {
  ...
  return <main>
    ...
    <section className="intro section-pad page-depth depth-intro"><div className="depth-orbit" aria-hidden="true"><i/><i/><b/></div><div className="container intro-layout">
    ...
    <section className="numbers page-depth depth-numbers"><div className="depth-grid" aria-hidden="true"/><div className="container number-grid">
    ...
    <section id="program" className="programs section-pad page-depth"><div className="container">
    ...
    <section className="feature-story depth-feature page-depth"><div className="molecule-overlay" aria-hidden="true"><i/><i/><i/><b/><b/></div><div className="feature-photo laboratory-photo"/><div className="feature-copy"><Reveal><span className="section-index light-index">CERITA DARI CAKRAWALA</span><p className="feature-quote">“Kami belajar bahwa ide kecil bisa punya dampak besar ketika dikerjakan bersama.”</p><p>Tim Arunika mengubah limbah kantin menjadi material bioplastik—proyek satu semester yang membawa mereka ke final kompetisi inovasi nasional.</p><Link className="button outline-light" href="/berita/pekan-proyek-siswa">Baca perjalanan mereka <ArrowRight size={18}/></Link></Reveal></div></section>

    <section className="facility-section section-pad depth-stage page-depth depth-facility"><div className="depth-axis" aria-hidden="true"/><div className="container facility-head"><Reveal><span className="section-index">03 / RUANG TUMBUH</span><h2>Kampus untuk <em>mencoba.</em></h2></Reveal><Reveal><p>Dirancang untuk fokus, kolaborasi, dan kemungkinan baru.</p></Reveal></div><div className="facility-track">{facilities.map((f,i)=><Reveal className="facility-card depth-card" key={f[0]}><div className="facility-img" style={{backgroundImage:`url('${f[2]}')`}}><span>0{i+1}</span></div><h3>{f[0]}</h3><p>{f[1]}</p></Reveal>)}</div></section>

    <section id="berita" className="news section-pad page-depth depth-news"><div className="depth-sheet" aria-hidden="true"/><div className="container"><Reveal className="news-head"><div><span className="section-index">04 / JURNAL SEKOLAH</span><h2>Yang sedang<br/><em>kami rayakan.</em></h2></div><a className="text-link" href="#berita">Lihat semua kabar <ArrowRight size={16}/></a></Reveal><div className="news-grid">{news.map((n,i)=><Reveal className={`news-item news-${i}`} key={n.slug}><Link href={`/berita/${n.slug}`}><div className="news-img" style={{backgroundImage:`url('${newsImages[i]}')`}}/><span>{n.date} · {i===0?"PRESTASI":i===1?"INOVASI":"AGENDA"}</span><h3>{n.title}</h3><p>{n.text}</p><ArrowRight/></Link></Reveal>)}</div></div></section>

    <section className="testimonial section-pad page-depth depth-quote"><div className="quote-sculpture" aria-hidden="true">“</div><div className="container testimonial-layout"><Quote size={64}/><Reveal><blockquote>“Di sini, anak kami tidak diminta menjadi sama dengan yang lain. Ia dibantu mengenali kekuatannya, berani bertanya, dan bertanggung jawab atas pilihannya.”</blockquote><p><strong>Rina Prameswari</strong><br/>Orang tua siswa kelas XI</p></Reveal><div className="testimonial-photo"/></div></section>

    <section id="kontak" className="ppdb-cta page-depth depth-portal"><div className="portal-rings" aria-hidden="true"><i/><i/><i/></div><div className="container cta-layout"><Reveal><span className="section-index light-index">PPDB 2026 / 2027</span><h2>Cakrawala berikutnya<br/>dimulai <em>di sini.</em></h2></Reveal><Reveal className="cta-side"><p>Pendaftaran gelombang pertama dibuka. Temukan lingkungan belajar yang tepat untuk langkah besar berikutnya.</p><Link className="button lime" href="/ppdb">Daftar PPDB sekarang <ArrowRight size={18}/></Link><span>Butuh bantuan? (0761) 555-0188</span></Reveal></div></section>
  </main>;
}
